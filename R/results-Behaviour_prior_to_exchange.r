...
# SETTINGS & DATA
    # do you want plots in R (PNG=FALSE) or as PNG (PNG = TRUE) or as EPS?
		PNG=FALSE #PNG = TRUE
		EPS=FALSE
...
# Figure 1ab EPS 
  ...
  # plot
	if(EPS == TRUE) {
	  setEPS()
      postscript(paste0(outdir,"Figure_1AB_02.eps"), width=1.85+0.6,height=1.5*2)
...
# model assumptions
	# Table A1a - calling - type
		  ...
		  # spatial autocorrelations - nest location
			...
			if(PNG == TRUE){dev.off()}
	# Table A1b - calling type * incubation period, type * sex
			  if(PNG == TRUE){png(paste(outdir,"model_ass/Table_A1b.png", sep=""), width=6,height=9,units="in",res=600)}else{dev.new(width=6,height=9)}	
		  	  par(mfrow=c(5,3),oma = c(0, 0, 1.5, 0) )
			  ...
			  # spatial autocorrelations - nest location
				...
				if(PNG == TRUE){dev.off()}
	# Table A1c - fly-off - type
			if(PNG == TRUE){png(paste(outdir,"model_ass/Table_A1c.png", sep=""), width=6,height=9,units="in",res=600)}else{dev.new(width=6,height=9)}	
		  	par(mfrow=c(5,3),oma = c(0, 0, 1.5, 0) )
			m= glmer(fly_bin ~ scale(log(obs_time))+type + (1|nest_ID), family = binomial,  bb_)
			...
			if(PNG == TRUE){dev.off()}	
	# Table A1d - fly-off type * incubation period	
			if(PNG == TRUE){png(paste(outdir,"model_ass/Table_A1d.png", sep=""), width=6,height=9,units="in",res=600)}else{dev.new(width=6,height=9)}	
		  	par(mfrow=c(5,3),oma = c(0, 0, 1.5, 0) )
...
# NOT USED Figure 1ab ALTEERNATIVE and within text info 
	  # run first 
	     # model predictions
	     	# calls
				m= glmer(call_i ~ type + (1|nest_ID), offset = log(obs_time/10),family = poisson,  bb_)# rate per 10 minute
		 ...
		 # raw data
			 ...
			 uw$m_ex_j=jitter(uw$m_ex,amount = 0.05) 
			 uw$m_non_j=jitter(uw$m_non,amount = 0.05) 

			 x=ddply(bb_,.(nest_ID, type), summarise,m=median(fly_bin), q1=quantile(fly_bin,0.25), q2= quantile(fly_bin,0.75), n = sum(n))
			 xw = reshape(data = x[, c('nest_ID','type', 'm', 'q1', 'q2', 'n')], idvar = "nest_ID", v.name = c("m", "q1", "q2", "n"), timevar = "type", direction = "wide", sep ='_')
			 xw$m_ex_j=jitter(xw$m_ex,amount = 0.025) 
			 xw$m_non_j=jitter(xw$m_non,amount = 0.025) 		
	  # within text info
			pt_ # prediction for calling rate before arrival
			pc_ # prediction for calling rate control
			pe # prediction for fly-off probability before arrival
	  ...
	  # plot
		if(PNG == TRUE) {png(paste(outdir,"Figure_1AB_alternative.png", sep=""), width=1.82,height=1.8*2,units="in",res=600) 
			}else{dev.new(width=1.82,height=1.8*2)}	
		
		par(mfrow=c(2,1),oma = c(1, 2.15, 0, 0))		
				
		# calls
			#par(mar=c(0.8,0.1,0.2,0.1),oma = c(1, 2, 1, 0),ps=12, mgp=c(1.2,0.35,0), las=1, cex=1, col.axis="black",font.main = 1, col.lab="black", col.main="black", fg="black", cex.lab=0.6,cex.main=0.7, cex.axis=0.5, tcl=-0.1,bty="n",xpd=TRUE)
			par(mar=c(0.8,0.1,1,0.1),ps=12, mgp=c(1.2,0.35,0), las=1, cex=1, col.axis="black",font.main = 1, col.lab="black", col.main="black", fg="black", cex.lab=0.6,cex.main=0.7, cex.axis=0.5, tcl=-0.1,bty="n",xpd=TRUE)
			plot(uw$m_ex_j~uw$m_non_j, xlim=c(0,3), ylim=c(0,3),xaxt='n',yaxt='n',type='n')
			
			#mtext(substitute(paste(bold('A'), " - Calling rate [calls/10min]")),side=3,line=0, cex=0.55)
			mtext(substitute(paste(bold('A'), " - Calling rate per 10min")), at = c(0.45), side = 3, line = 0, cex=0.6)
			axis(1, at=seq(0,3,by=0.5), lwd = 0.35, mgp=c(0,-0.20,0))
			axis(2, at=seq(0,3,by=0.5), lwd = 0.35)
			#mtext("Incubation day\n",side=1,line=0.9, cex=0.55, las=1, col='black')
			mtext("Before arrival\n[incubation prior to exchange]",side=2,line=1.1, cex=0.55, las=3, col='black')
			#text(7.1,3, expression(bold('A')),cex=0.6)
			#text(7.1,1, expression(bold('B')),cex=0.6)	
			#mtext("Before nest relief",side=2,line=1, cex=0.55, las=3)
				
			
			arrows(x0=uw$m_non_j, y0=uw$q1_ex,x1=uw$m_non_j, y1=uw$q2_ex, code = 0, col=col_p, angle = 90, length = .025, lwd=0.5, lty=1)
			arrows(y0=uw$m_ex_j, x0=uw$q1_non,y1=uw$m_ex_j, x1=uw$q2_non, code = 0, col=col_p, angle = 90, length = .025, lwd=0.5, lty=1)
			points(x = uw$m_non_j, y = uw$m_ex_j, bg=adjustcolor(col_pb, alpha.f = 0.4), col=adjustcolor(col_p, alpha.f = 0.8), pch =21, cex = 0.6)
			#symbols(x = uw$m_non_j, y = uw$m_ex_j, circles=sqrt(((uw$n_non + uw$n_ex)/2)/pi),inches=0.14/1.75,bg=col_pb, fg=col_p,add=TRUE) #
			lines(x = c(0,3), y = c(0,3), lty = 3, col = "grey70") 

			# add predictions + 95%CI
				arrows(x0=pc_$pred, y0=pt_$lwr,x1=pc_$pred, y1=pt_$upr, code = 0, col="red", angle = 90, length = .025, lwd=1.5, lty=1)
				arrows(x0=pc_$lwr, y0=pt_$pred,x1=pt_$upr, y1=pt_$pred, code = 0, col="red", angle = 90, length = .025, lwd=1.5, lty=1)
				points(x = pc_$pred, y=pt_$pred,pch=20, cex=0.9,col="red")
				
			# legend
				text(c(.6),c(2.78),labels=c('Median & IQR'), xpd=TRUE, cex=0.5, xpd=FALSE,col='grey56', adj = c(0,0.5)) 
				text(c(.6),c(2.6),labels=c('Predictions & 95%CI'), xpd=TRUE, cex=0.5, xpd=FALSE,col='red', adj = c(0,0.5)) 
					
		# fly-offs
			par(mar=c(0.8,0.1,1,0.1),ps=12, mgp=c(1.2,0.35,0), las=1, cex=1, col.axis="black",font.main = 1, col.lab="black", col.main="black", fg="black", cex.lab=0.6,cex.main=0.7, cex.axis=0.5, tcl=-0.1,bty="n",xpd=TRUE)	

			plot(xw$m_ex_j~xw$m_non_j, xlim=c(0,1), ylim=c(0,1),xaxt='n',yaxt='n',type='n')

			mtext(substitute(paste(bold('B'), " - Fly-off probability")), at = c(0.02), side = 3, line = 0, cex=0.6)
			axis(1, at=seq(0,1,by=0.2), lwd = 0.35, mgp=c(0,-0.20,0))
			axis(2, at=seq(0,1,by=0.2), lwd = 0.35)
			mtext("Control\n[during other times of incubation]",side=1,line=0.9, cex=0.55, las=1, col='black')
			mtext("Before arrival\n[incubation prior to exchange]",side=2,line=1.1, cex=0.55, las=3, col='black')
				
			arrows(x0=xw$m_non_j, y0=xw$q1_ex,x1=xw$m_non_j, y1=xw$q2_ex, code = 0, col=col_p, angle = 90, length = .025, lwd=0.5, lty=1)
			arrows(y0=xw$m_ex_j, x0=xw$q1_non,y1=xw$m_ex_j, x1=xw$q2_non, code = 0, col=col_p, angle = 90, length = .025, lwd=0.5, lty=1)
			points(x = xw$m_non_j, y = xw$m_ex_j, bg=adjustcolor(col_pb, alpha.f = 0.4), col=adjustcolor(col_p, alpha.f = 0.8), pch =21, cex = 0.6)
			#symbols(x = xw$m_non_j, y = xw$m_ex_j, circles=sqrt(((xw$n_non + xw$n_ex)/2)/pi),inches=0.14/1.75,bg=col_pb, fg=col_p,add=TRUE) #
			lines(x = c(0,1), y = c(0,1), lty = 3, col = "grey70") 

			# add predictions + 95%CI
				arrows(x0=pn$pred, y0=pe$lwr,x1=pn$pred, y1=pe$upr, code = 0, col="red", angle = 90, length = .025, lwd=1.5, lty=1)
				arrows(x0=pn$lwr, y0=pe$pred,x1=pe$upr, y1=pe$pred, code = 0, col="red", angle = 90, length = .025, lwd=1.5, lty=1)
				points(x = pn$pred, y=pe$pred,pch=20, cex=0.9,col="red")
...
# NOT IN THE MS Cases where calling/flying occurred - was it closer to the exchange start? and if so is this sex specific
	...
	# model ass - Table S2a - calling simple
		...
		if(PNG == TRUE){dev.off()}		
	# model ass - Table S2b - calling sex
		...
		if(PNG == TRUE){dev.off()}
	# model ass - Table S2c - calling simple
		...
		if(PNG == TRUE){dev.off()}	
	# model ass - Table S2d - calling sex
